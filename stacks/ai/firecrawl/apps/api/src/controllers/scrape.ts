...
export async function scrapeController(req: Request, res: Response) {
  try {
    ...
    try {
      let creditsCheckSuccess = true;
      let creditsCheckMessage = "";
      if (process.env.USE_DB_AUTHENTICATION === "false") {
        const { success: creditsCheckSuccessTemp, message: creditsCheckMessageTemp } =
          await checkTeamCredits(team_id, 1);
          creditsCheckSuccess = creditsCheckSuccessTemp;
          creditsCheckMessage = creditsCheckMessageTemp;
       }
