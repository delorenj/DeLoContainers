...
## Local Development Environment Setup (DeLoContainers)

### Docker Compose Configuration
We've configured a local development environment for PotPie with the following container services:
- `momentum` (Main PotPie App)
- `postgres` (Database)
...
- `isDevelopmentMode=enabled`
- `ENV=development`
- `defaultUsername=delorenj`
- Database configs (PostgreSQL, Redis, Neo4j)
- AI Model Configuration using OpenRouter

### Connectivity
- Application runs on `localhost:8001`
- Development mode authentication is enabled
- API endpoints are accessible at `/api/v1/`
...
- `/api/v1/project/{project_id}/message/`

### Future Enhancements
- Full authentication system setup
