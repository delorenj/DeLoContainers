...
export async function supaBillTeam(team_id: string, credits: number) {
  // Modified to bypass billing for self-hosted instances
  if (team_id === "preview" || process.env.USE_DB_AUTHENTICATION === "false" || team_id.length > 40) {
    return { success: true, message: "Self-hosted or preview instance, no credits used" };
  }
...
export async function supaCheckTeamCredits(team_id: string, credits: number) {
  // Modified to bypass credit checks for self-hosted instances
  if (team_id === "preview" || process.env.USE_DB_AUTHENTICATION === "false" || team_id.length > 40) {
    return { success: true, message: "Self-hosted or preview instance, unlimited credits" };
  }
